...
GLuint SetupPhase345DataForRendering()
{
	...
	glGenBuffers(1, &index_vbo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index_vbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 77535 * sizeof(GLuint), tri_indices, GL_STATIC_DRAW);
...
// cameraloc  : is the location of the camera
...
"out vec4 frag_color;\n"
"float Interpolate(float A, float B, float FA, float FB, float X) {\n"
"  float FX = FA + (X - A) / (B - A) * (FB - FA);\n"
"  return FX;\n"
"}\n"
"void main() {\n"
...
"  if(data < 4.5){\n"
"   float lerp = Interpolate(1, 4.5, .25, 1, data);\n"
"   frag_color = vec4(lerp, lerp, 1.0, 1.0);\n"
"  }else{\n"
"   float lerp = Interpolate(4.5, 6, 1, .25, data);\n"
"   frag_color = vec4(1.0, lerp, lerp, 1.0);\n"
"  }\n"
#endif

#ifdef PHASE5
// Update frag_color by mixing the shading factor
"  frag_color = frag_color * vec4(shading_amount, shading_amount, shading_amount, 1.0);\n"
#endif
